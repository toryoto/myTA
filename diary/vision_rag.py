...
def build_image_embeddings_cache(user_id):
    ...
    days_with_images = Day.objects.filter(
        author_id=user_id, image__isnull=False
    ).exclude(image="")
...
def search_days_by_image_content(
    query_text, user_id, top_k=10, similarity_threshold=0.3
):
    """
    画像内容に基づいて日記を検索
    ...
    # デバッグ情報：類似度スコアを表示
    print(
        f"類似度スコア: min={similarity_scores.min():.4f}, max={similarity_scores.max():.4f}, mean={similarity_scores.mean():.4f}"
    )
